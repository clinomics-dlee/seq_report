...
/* 01. Handle Scrollbar
------------------------------------------------ */
//const _ethAddr = "0xa33dA3d1ED5Fdd183a49d40871770637Eb98F885";
var _delay = 250;
var _notAllowed = new Array();
...
$('#logout').bind('click', function(){
    if(localStorage.getItem('public')) localStorage.removeItem('public');
    $('#logoutForm').submit();
});
 ...
 * @param e
 */
$('.nav-side').find('li').find('a').each(function(i, e){
    if($(this).attr('href') == $(location).attr('pathname') || $(location).attr('pathname').indexOf($(this).attr('href')) != -1){
 ...
 * @returns
 */
var setLocale = function(_form){
    $.getJSON("https://ipinfo.io?token="+_ipToken, function(data) {
        _form.append("<input type=\"hidden\" value=\""+data.ip+"\"      name=\"connectIp\">");
        _form.append("<input type=\"hidden\" value=\""+data.region+"\"  name=\"connectRegion\">");
        _form.append("<input type=\"hidden\" value=\""+data.country+"\" name=\"connectCountry\">");
        _form.append("<input type=\"hidden\" value=\""+data.city+"\"    name=\"connectCity\">");
    });
}
...
var handleCountdownTimer = function() {
    ...
    var finalDate = new Date(Date.UTC(2018, 08, 10))
    //180909 ksko ICO presale 기간 수정 (시간부분 임시 추가)
    /*finalDate.setUTCHours(finalDate.getUTCHours() - 18);
    finalDate.setUTCMinutes(finalDate.getUTCMinutes() - 41);
    finalDate.setUTCHours(hours || 0);
    finalDate = new Date(newYear.getFullYear() + 1, 1 - 1, 1);*/
    //180910 skpark d-day 완료시 timezone hide
    $('#timezone').countdown({
        until: finalDate,
        layout: '{dn}{dl} {hn}:{mn}:{sn}',
        alwaysExpire : true,
        onExpiry : expiredSales
    });
};

function expiredSales(){
    $('#timezone').closest('small').hide();
}
...
/* ------------------------------------------------ */
//중복 저장 방지
function doPrevent(_this, _flag){
	var $this = $(_this);
	if(_flag){
		$('form').find('input,select').attr('readonly', true);
		_submitted = true;
	    var loadingText = '<i class=\'fas fa-circle-notch fa-spin\'></i> '+_locale.message.loading;
	    if ($(_this).html() !== loadingText) {
	    	$this.data('original-text', $(_this).html());
 ...
 * @param item
 */
$('#_token').children().bind('click', function(index, item){
    Cookies.set('_token', $(this).text() == 'JGX' ? '0' : '1');
    $('#_currentToken').text($(this).text());
});
